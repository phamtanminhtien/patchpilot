...
Runtime config loads from OS environment variables, falling back to a local `.env` file in the process working directory. OS environment variables override `.env` values. `PATCHPILOT_ADDR` controls the backend listen address, `PATCHPILOT_ALLOWED_ROOTS` controls the OS path-list of allowed workspace roots, `PATCHPILOT_STATIC_DIR` optionally serves the built frontend, `PATCHPILOT_LOG_FORMAT` selects colorized `console` logs or `json` logs, `PATCHPILOT_ADMIN_TOKEN` configures the single-user login secret, `PATCHPILOT_OPENAI_API_KEY` configures the OpenAI-compatible provider secret, `PATCHPILOT_OPENAI_BASE_URL` optionally overrides the OpenAI-compatible API base URL, and `PATCHPILOT_DB_PATH` overrides the SQLite database path. When `PATCHPILOT_DB_PATH` is unset, `PATCHPILOT_DATA_DIR` controls the directory for `patchpilot.db`; otherwise `~/.patchpilot/patchpilot.db` is used.
